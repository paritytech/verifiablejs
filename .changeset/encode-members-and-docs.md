...
- `validate_with_commitment` is now documented in both READMEs as the recommended local pre-flight check before an on-chain submission — it validates a proof against the 768-byte ring root (`MembersCommitment`) the chain exposes.
